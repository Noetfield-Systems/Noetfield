...
# nf-0905 — Update docs/ops/NOETFIELD_AGENT_TEAM_SYNC revision row only if needed
...
```
PLAN WITH NO ASF — Noetfield agent prompt nf-0905. Update docs/ops/NOETFIELD_AGENT_TEAM_SYNC revision row only if needed (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read docs/ops/AGENT_READ_LINKS_LOCKED_v1.md and os/SHIP_NOW.md. Tag new docs [NF-LOCAL-REPO-AGENT] + authored_at. Do not edit [NF-CLOUD-AGENT] sections. Post-flight: make verify-gtm, cursor-reply with reported_at, ingest, sync-sourceA.
```
