...
# nf-0920 — Reject edits to SinaPromptOS code grep in agent session
...
```
PLAN WITH NO ASF — Noetfield agent prompt nf-0920. Reject edits to SinaPromptOS code grep in agent session (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read docs/ops/AGENT_READ_LINKS_LOCKED_v1.md and os/SHIP_NOW.md. Tag new docs [NF-LOCAL-REPO-AGENT] + authored_at. Do not edit [NF-CLOUD-AGENT] sections. Post-flight: make verify-gtm, cursor-reply with reported_at, ingest, sync-sourceA.
```
