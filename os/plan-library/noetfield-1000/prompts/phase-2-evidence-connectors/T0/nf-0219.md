---
id: nf-0219
phase: phase-2-evidence-connectors
tier: T0
priority: P0
status: backlog
lane: noetfield
library: noetfield-1000-locked
agent_tag: nf-local-repo-agent
slot: 18
generator: scripts/generate-noetfield-1000-prompts.py
locked: true
updated_at: 2026-06-06
---

# nf-0219 — Document mock vs real M365 in LOCAL_DEV.md one paragraph

**Phase:** `phase-2-evidence-connectors` — Evidence index, M365/Google connectors, intake  
**Tier:** `T0` — Do now. Minimal scope. Run verify gate before close.

## Agent prompt (copy to chat)

```
PLAN WITH NO ASF — Noetfield agent prompt nf-0219. Document mock vs real M365 in LOCAL_DEV.md one paragraph (Do now. Minimal scope. Run verify gate before close.). Pre-flight: read docs/ops/AGENT_READ_LINKS_LOCKED_v1.md and os/SHIP_NOW.md. Tag new docs [NF-LOCAL-REPO-AGENT] + authored_at. Do not edit [NF-CLOUD-AGENT] sections. Post-flight: make verify-gtm, cursor-reply with reported_at, ingest, sync-sourceA.
```

## Task

Document mock vs real M365 in LOCAL_DEV.md one paragraph

## Sources (read first)

- `docs/reference/GOVERNANCE_SOURCES_BOOK_v1.md`
- `scripts/copilot-pilot-e2e.sh`
- `governance-console/frontend/app/workspace/connectors/`
- `docs/strategy/NOETFIELD_GTM_60_DAY_LOCKED_v1.md` (60-day fence)
- `os/plan-library/noetfield-1000/SOURCES_INDEX.yaml`

## Verify

```bash
make verify-gtm
```

## Closeout

1. Set `status: done` in this file front matter
2. `reports/cursor-reply-latest.txt` with `agent_tag` + `reported_at`
3. `~/Desktop/SinaPromptOS/scripts/ingest-cursor-reply.sh noetfield reports/cursor-reply-latest.txt`
4. `./scripts/sync-sourceA-desktop.sh`
5. `python3 scripts/sync-noetfield-plans-status.py` (optional nf-future mirror)
