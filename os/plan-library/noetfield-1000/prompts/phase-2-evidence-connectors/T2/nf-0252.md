...
# nf-0252 — Harden evidence hash on ingest in connector service
...
```
PLAN WITH NO ASF — Noetfield agent prompt nf-0252. Harden evidence hash on ingest in connector service (Quarterly hardening. Refactor only if verify stays green.). Pre-flight: read docs/ops/AGENT_READ_LINKS_LOCKED_v1.md and os/SHIP_NOW.md. Tag new docs [NF-LOCAL-REPO-AGENT] + authored_at. Do not edit [NF-CLOUD-AGENT] sections. Post-flight: make verify-gtm, cursor-reply with reported_at, ingest, sync-sourceA.
```
