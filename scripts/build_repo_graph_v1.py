...
# Noetfield is a sprawling repo (~90 top-level dirs — website routes + governance
# layers). Rather than hardcode, AUTO-DISCOVER top-level subsystems, excluding only
# build/dependency/cache noise. This guarantees the library (os/plan-library,
# L2-knowledge) and memory (.cursor/agent-memory) dirs are always covered.
_EXCLUDED_TOP = {
    ".git", "node_modules", "__pycache__", ".pytest_cache", ".wrangler",
    "dist", "build", ".next", ".cache", "venv", ".venv", "graph-out",
    "_archive", "www-pages-dist", "var", "vendor", "tmp", "noetfield.egg-info",
    ".turbo", ".vercel",
}
