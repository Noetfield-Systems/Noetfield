...
sync_pages_secrets() {
  local key value
  for key in RESEND_API_KEY INTAKE_EMAIL_FROM INTAKE_EMAIL_TO OPENROUTER_API_KEY TELEGRAM_NOETFIELD_OPS_BOT_TOKEN TELEGRAM_OPS_CHAT_ID DISCORD_OPS_WEBHOOK_URL AUTH_OAUTH_PROVIDERS RUNWAY_RUNTIME_API_SECRET RUNWAY_RUNTIME_BASE_URL RUNWAY_RUNTIME_KEY_ID; do
    value="${!key:-}"
    if [[ -z "$value" ]]; then
