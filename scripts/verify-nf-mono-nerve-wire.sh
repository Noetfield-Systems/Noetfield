...
TF_WIRE=""
for cand in \
  "$HOME/Desktop/Noetfield-Systems/TrustField-Technologies/scripts/tf_fleet_live_wire_v1.py" \
  "$HOME/Desktop/TrustField Technologies/scripts/tf_fleet_live_wire_v1.py" \
  "$HOME/Desktop/TrustField-Technologies/scripts/tf_fleet_live_wire_v1.py"
do
  if [[ -f "$cand" ]]; then TF_WIRE="$cand"; break; fi
done
if [[ -n "$TF_WIRE" ]]; then
