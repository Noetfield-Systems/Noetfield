...
The page is live at a public address — preview ready, customer delivered.
Summit Ridge Physical Therapy: services, team, and the intake form.

Same session. No second ask. The automation lane delivers a verified workflow file in Files.

Page and workflow from one sentence — front door through workspace to delivery.
Edited live session: waiting is not shown in real time. The run receipt is on the Evidence Lab.
""".strip()
...
def capture_delivered(browser, storage: dict, project_id: str, slug: str) -> None:
    ...
    record_segment(browser, storage, cockpit, CAPTURE / "08-files-workflow.mp4", 10.0)
