...
| 2026-07-09 | Vercel (team `the-777-foundation`, project `noetfield`) is live/canonical for www | Superseded: Vercel fully retired from the Noetfield stack. `www.noetfield.com` (and apex `noetfield.com`) now canonically served by Cloudflare Pages, project `noetfield-www`, via `scripts/deploy-www-cloudflare.sh`. Backend/platform stays on Railway (`platform.noetfield.com`, `api.noetfield.com`). The Vercel cloud project and all Vercel-provisioning scripts have been deleted from the repo — no live fallback or rollback target remains. | `scripts/deploy-www-cloudflare.sh`, `docs/ops/CF_WWW_PROXY_LOCKED_v1.md`, `docs/ops/CLOUD_INVENTORY_LOCKED_v1.md` |
