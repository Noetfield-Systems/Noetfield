"""Trust Brief diligence factory runtime."""
...
FACTORY_ID = "trust_brief_diligence_v1"


class TrustBriefDiligenceCommand(BaseModel):
...
class TrustBriefDiligenceRuntime:
    def __init__(
        self,
        *,
    ...
    async def step_signal_ingest(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
        signal, _ = await self._signal_pipeline.ingest(
            IngestSignalCommand(
                ...
                signal_type="trust_brief_diligence_signal",
                payload=state.command.signal_payload,
                provenance={"module": "trust_brief", "ingestion": "diligence"},
    ...
    async def step_graph_mutate(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
        mutation = await self._graph_mutations.mutate_relationship(
            GraphMutationCommand(
                ...
                relationship_type="trust_brief_diligence_for",
                confidence_delta=0.2,
                reason="Trust Brief diligence signal linked to portco/thesis graph.",
                actor_id=state.command.submitted_by,
            )
        )
    ...
    async def step_graph_reflect(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
    ...
    async def step_inspector_collaborate(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
        ...
        inspector_run = await self._inspector_execution_loop.run_once(
            InspectorCollaborationCommand(
                ...
                objective="Assess investor diligence governance, risk, and readiness signals.",
    ...
    async def step_policy_evaluate(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
        ...
        decision = await self._golden_edge.evaluate(
            GoldenEdgeEvaluateRequest(
                tenant_id=state.command.tenant_id,
                organization_id=state.command.organization_id,
                action="run_trust_brief_diligence",
                resource_type="trust_brief_diligence_run",
                resource_id=resource_id,
                actor_id=state.command.submitted_by,
                confidence=0.8,
                payload={"module": "trust_brief", "run_id": str(state.run_id)},
            )
        )
        state.policy_decision = decision
        return state

    async def step_workflow_govern(self, state: TrustBriefPipelineState) -> TrustBriefPipelineState:
        ...
        workflow = await self._workflow_state_machine.start(
            WorkflowInstance(
                tenant_id=state.command.tenant_id,
                organization_id=state.command.organization_id,
                workflow_type="trust_brief_diligence_review",
        ...
        workflow = await self._workflow_state_machine.transition(
            WorkflowTransitionCommand(
                ...
                reason="Trust Brief IC appendix requires human approval before export.",
            )
        )
