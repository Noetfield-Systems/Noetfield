...
from noetfield_legal_review import (
    FACTORY_ID,
    LegalReviewCommand,
    LegalReviewPipelineState,
    LegalReviewRuntime,
)

from .exceptions import FactoryValidationError
from .loader import factory_node_ids, load_factory_spec
from .models import FactoryStatus, LegalFactoryOutput, LegalFactoryRunRequest
from .nodes import factory_actor, intake_validate_legal, package_legal_deliverable


class LegalFactoryRunner:
    FACTORY_ID = FACTORY_ID

    def __init__(
        self,
        *,
        legal_runtime: LegalReviewRuntime,
        event_bus: AsyncEventBus,
        audit_store: Any,
        graph_store: Any,
        governance_runtime: Any,
    ) -> None:
        self._runtime = legal_runtime
    ...
    async def run(self, request: LegalFactoryRunRequest) -> LegalFactoryOutput:
    ...
    async def _execute(self, run_id: UUID, command: LegalReviewCommand) -> LegalFactoryOutput:
        ...
        validated = intake_validate_legal(command)
        state = LegalReviewPipelineState(run_id=run_id, command=validated)
        ...
        if (
            ...
            packaged = await package_legal_deliverable(
            ...
            return LegalFactoryOutput(
                factory_id=self.FACTORY_ID,
                run_id=run_id,
                factory_status=FactoryStatus.VETOED,
                policy_review_package=packaged["policy_review_package"],
