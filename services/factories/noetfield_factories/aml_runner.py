"""AML governance trace factory runner."""
...
from noetfield_aml_trace import (
    FACTORY_ID,
    AmlGovernanceTraceCommand,
    AmlGovernanceTracePipelineState,
    AmlGovernanceTraceRuntime,
)

from .exceptions import FactoryValidationError
from .loader import factory_node_ids, load_factory_spec
from .models import AmlFactoryOutput, AmlFactoryRunRequest, FactoryStatus
from .nodes import factory_actor, intake_validate_aml, package_aml_deliverable


class AmlFactoryRunner:
    FACTORY_ID = FACTORY_ID

    def __init__(
        self,
        *,
        aml_runtime: AmlGovernanceTraceRuntime,
        event_bus: AsyncEventBus,
        audit_store: Any,
        graph_store: Any,
        governance_runtime: Any,
    ) -> None:
        self._runtime = aml_runtime
    ...
    async def run(self, request: AmlFactoryRunRequest) -> AmlFactoryOutput:
    ...
    async def _execute(
        self, run_id: UUID, command: AmlGovernanceTraceCommand
    ) -> AmlFactoryOutput:
        ...
        validated = intake_validate_aml(command)
        state = AmlGovernanceTracePipelineState(run_id=run_id, command=validated)
        ...
        if (
            ...
            packaged = await package_aml_deliverable(
            ...
            return AmlFactoryOutput(
                factory_id=self.FACTORY_ID,
                run_id=run_id,
                factory_status=FactoryStatus.VETOED,
                aml_trace_audit_package=packaged["aml_trace_audit_package"],
